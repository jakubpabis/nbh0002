<?php
/**
 * Sative functions and definitions
 *
...
if ( ! function_exists( 'sative_setup' ) ) :
	...
	function sative_setup() {
		 ...
		 * If you're building a theme based on Sative, use a find and replace
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'sative' ),
			)
		);
...
function sative_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'sative_content_width', 640 );
}
...
function sative_scripts() {
	wp_enqueue_style( 'sative-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'sative-style', 'rtl', 'replace' );

	wp_enqueue_script( 'sative-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
